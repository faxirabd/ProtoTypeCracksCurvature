...
namespace ProtoTypeCurvature_V3
{
    class Curvature
    {
        ...
        //to cut all bins higher than threshold!
        public double[,] CutFewFirstBins(double[,] histogram, double threshold)
        {
            for (int i = 0; i < histogram.GetLength(0); i++)
            {
                if (histogram[i, 1] > threshold)
                    histogram[i, 1] = 0.00000001;
            }

            return histogram;
        }

        public double[,] calculate(Point[][] segments, int samplestep, float stepsize, int curvatureRange)
        {
            ...
            for (int i = 0; i < segments.GetLength(0); i++)
            {
                ...
                if (samplestep >= segments[i].GetLength(0))
                    continue;
                p1.X = segments[i][samplestep].X;
                p1.Y = segments[i][samplestep].Y;
                for (int j = (2 * samplestep); j < segments[i].GetLength(0);)
                {
                    p2.X = segments[i][j].X;
                    p2.Y = segments[i][j].Y;
                    ...
                    PointF firstDerivative = new PointF();
                    PointF secondDerivative = new PointF();
                    firstDerivative.X = (p1.X - p.X) / samplestep;
                    firstDerivative.Y = (p1.Y - p.Y) / samplestep;
                    secondDerivative.X = (p2.X - (2.0F * p1.X) + p.X) / samplestep;
                    secondDerivative.Y = (p2.Y - (2.0F * p1.Y) + p.Y) / samplestep;
                    ...
                    double curv = Math.Round((curvature * stepsize));
                    int hisvalue = (int)curv;
            ...
            for (int i = 0; i < histogram.GetLength(0); i++)
            {
                histogram[i, 0] = Math.Round(i / stepsize, 3);//to get the original values back
            }

            return histogram;
        }

        public double[,] calculate(PointF[][] segments, int samplestep, float stepsize, int curvatureRange)
        {
            double[,] histogram = new double[curvatureRange, 2];
        ...
        public float[] calculateOnlyFreq(Point[][] segments, int samplestep, float stepsize, int curvatureRange)
        {
            float[] histogram = new float[curvatureRange];
        ...
        public double[,] calculate(Point[][] segments, int samplestep, float stepsize, int curvatureRange, string rawdatafile)
        {
            //we want tpo write the curvature values into a textfile to do some experiments.
            StreamWriter sw = new StreamWriter(rawdatafile);
            ...
            for (int i = 0; i < segments.GetLength(0); i++)
            {
                ...
                for (int j = (2 * samplestep); j < segments[i].GetLength(0);)
                {
                    ...
                    if (hisvalue < curvatureRange) //&& (hisvalue > 0))
                    {
                        histogram[hisvalue, 1]++;
                        if (curvature == 0.0)
                            curvature = 0.00000001;
                        sw.WriteLine(curvature);
                    }
        ...
        public double[,] calculate(PointF[][] segments, int samplestep, float stepsize, int curvatureRange, string rawdatafile)
        {
            //we want tpo write the curvature values into a textfile to do some experiments.
            StreamWriter sw = new StreamWriter(rawdatafile);
        ...
        public void calculatev2(string filepath, int stepsize, int curvatureRange)
        {
            ...
            while ((line = strd.ReadLine()) != null)
            {
                ...
                do
                {
                    ...
                    double curv = Math.Round((curvature * 30.0));
                    int hisvalue = (int)curv;
                    if (hisvalue < curvatureRange)
                        histogram[hisvalue]++;
            ...
            for (int i = 0; i < histogram.Length; i++)
            {
                double x = Math.Round(i / 30.0, 3);//to get the original values back
                int y = histogram[i];
                sw.WriteLine(x + ", " + y);
            }
